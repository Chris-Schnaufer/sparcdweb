...
export default function LoginAgain({message, timeoutSec, onCancelTimeout, onTimedOut, onLogout}) {
  ...
  const [countdownId, setCountdownId] = React.useState(null);   // The timeout ID for the countdown timeer
  ...
  const clickListener = React.useCallback(() => {
    ...
    if (countdownId !== null) {
      ...
      setCountdownId(null);
      setRemainingSec(10000);
    }
  }, [countdownId, setCountdownId, setRemainingSec]);
  ...
  }, [countdownStartRef,  setCountdownId, setRemainingSec, timeoutSec]);

  // Manages when the timer reaches zero
  React.useEffect(() => {
    if (!timeoutSec || (timeoutSec && !remainingSec)) {
      onTimedOut();
    }
   ...
   * @return {object} Returns the UI to render
   */
  ...
  // Start off the countdown timer if needed
  if (countdownId === null) {
      ...
      >
        <div style={{backgroundColor:'rgb(0,0,0,0.8)', border:'1px solid grey', borderRadius:'15px', padding:'25px 10px'}}>
          <Grid container direction="column" alignItems="center" justifyContent="center" >
            { remainingSec > 0 ? 
              <React.Fragment>
                <Typography gutterBottom variant="body" sx={{color:"white"}}>
                  You will be logged out in
                </Typography>
                <Typography variant="h4" component="h4" sx={{padding:"20px 0px"}}  sx={{color:"white"}}>
